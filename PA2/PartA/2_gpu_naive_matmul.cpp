...
__global__ void matmul_gpu_naive(const float *a, const float *b, float *c, int n){
    int row, col;
...
int main(int argc, char* argv[]){
    ...
    printf("=====================================\n");
    printf("Starting gpu naive matmul for N=%d\n", n);
    const auto start_time = std::chrono::system_clock::now();
    // matmul_cpu(a, b, c, n);
    matmul_gpu_naive<<<grid, block>>>(da, db, dc, n);
